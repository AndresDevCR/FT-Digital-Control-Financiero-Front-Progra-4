...
export default function UserList() {
  ...
  const fetchUser = async () => {
    try {
      const response = await axios.get('https://control-financiero.herokuapp.com/api/v1/user', {
        headers: {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      setUser((prevUser) => prevUser.filter((userItem) => userItem.id !== id));
              ...
              {filteredRows
                ...
                .map((userItem) => (
                  <TableRow key={userItem.id}>
                    ...
                    <TableCell>
                      {userItem.company_start_date
                        ? userItem.company_start_date.split('T')[0]
                        : 'Fecha de inicio no asignada'}
                    </TableCell>

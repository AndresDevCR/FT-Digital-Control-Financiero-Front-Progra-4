...
const validationSchema = Yup.object().shape({
  ...
  primary_phone_number: Yup.string().max(30, 'El nombre debe tener como máximo 30 caracteres'),
  secondary_phone_number: Yup.string().max(30, 'El nombre debe tener como máximo 30 caracteres'),
  city: Yup.string().max(30, 'El nombre debe tener como máximo 30 caracteres'),
  state: Yup.string().max(30, 'El nombre debe tener como máximo 30 caracteres'),
  country: Yup.string().max(30, 'El nombre debe tener como máximo 30 caracteres'),
...
export default function CompanyForm() {
  ...
  const handleSubmit = async (values) => {
    try {
      const response= await axios.post('https://control-financiero.herokuapp.com/api/v1/company', values, {
        headers: {
          Authorization: `Bearer ${accessToken}`, // Incluye el token de autenticación en el encabezado
        },
      });
      if (response.status === 201) {
        enqueueSnackbar('Agregado correctamente a las compañías', { variant: 'success' });
        router.push('/dashboard/company/list');
      }
    } catch (error) {
      enqueueSnackbar('Error al agregar la compañía', { variant: 'error' });
    }
  };
